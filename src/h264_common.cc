...
bool more_rbsp_data(BitBuffer* bit_buffer) {
    ...
    // (1) We know that rbsp_trailing_bits() itself is limited to at most 1 byte.
    // Its definition is:
    // > rbsp_trailing_bits() {
    ...
    //
    // In real-world streams, however, some encoders append extra zero bytes
    // (0x00) after rbsp_trailing_bits() as padding. These bytes are not part
    // of the RBSP syntax, but they increase RemainingBitCount(). We treat such
    // trailing all-zero bytes as padding and ignore them when deciding whether
    // there is more RBSP data before rbsp_trailing_bits().
    //
